...
class EkgParameterDAOImpl(EkgParameterDAO):
    __PARAMETER_ID, __TIME, __HR, __HRV, __RR, __ANALYSIS_CODE = 0, 1, 2, 3, 4, 5
    __instance = None
    ...
    def find_eeg_signal_by_analysis_code(self, analysis_code: str) -> EkgParameterComposite | None:
        self.__connection = DbConnectionImpl.get_instance()
        sql = "SELECT * FROM ekg_parameters WHERE analysis_code = '" + analysis_code + "'"
        ...
        try:
            for row in self.__result_set.fetchall():
                self._create_ekg_parameter(row)
    ...
    def add_ekg_parameter(self, ekg_parameter: EkgParameterData):
        self.__connection = DbConnectionImpl.get_instance()
        sql = f"INSERT INTO ekg_parameters (time, hr, hrv, rr_interval, analysis_code) VALUES ({ekg_parameter.get_time()}, {ekg_parameter.get_heart_rate()}, {ekg_parameter.get_heart_rate_variability()}, {ekg_parameter.get_rr_interval()}, '{ekg_parameter.get_analysis_code()}')"
        db_operation_executor = DbOperationExecutorImpl()
    ...
    def delete_ekg_parameter(self, ekg_parameter: EkgParameterData):
        self.__connection = DbConnectionImpl.get_instance()
        sql = "DELETE FROM ekg_parameters WHERE analysis_code = '" + ekg_parameter.get_analysis_code() + "'"
        db_operation_executor = DbOperationExecutorImpl()
        db_operation = DbUpdateOperationImpl(sql)
        row_count = db_operation_executor.execute_write_operation(db_operation)
        self.__connection.close_connection()
    ...
    def _create_ekg_parameter(self, row: tuple) -> None:
        ...
        self.__ekg_parameter.set_analysis_code(row[self.__ANALYSIS_CODE])
