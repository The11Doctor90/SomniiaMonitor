...
class EegSignalDAOImpl(EegSignalDAO):
    __SIGNAL_ID, __TIME, __CHANNEL_1, __CHANNEL_2, __CHANNEL_3, __ANALYSIS_CODE = 0, 1, 2, 3, 4, 5
    __instance = None
    ...
    def find_eeg_signal_by_analysis_code(self, analysis_code: str) -> EegSignalComposite | None:
        self.__connection = DbConnectionImpl.get_instance()
        sql = "SELECT * FROM eeg_signals WHERE analysis_code = '" + analysis_code + "'"
    ...
    def add_eeg_signal(self, eeg_signal: EegSignalData):
        self.__connection = DbConnectionImpl.get_instance()
        sql = f"INSERT INTO eeg_signals (time, channel_1, channel_2, channel_3, analysis_code) VALUES ({eeg_signal.get_time()}, {eeg_signal.get_first_channel()}, {eeg_signal.get_second_channel()}, {eeg_signal.get_third_channel()}, '{eeg_signal.get_analysis_code()}')"
        db_operation_executor = DbOperationExecutorImpl()
    ...
    def delete_eeg_signal(self, eeg_signal: EegSignalData):
        self.__connection = DbConnectionImpl.get_instance()
        sql = "DELETE FROM eeg_signals WHERE analysis_code = '" + eeg_signal.get_analysis_code() + "'"
        db_operation_executor = DbOperationExecutorImpl()
        db_operation = DbUpdateOperationImpl(sql)
        row_count = db_operation_executor.execute_write_operation(db_operation)
        self.__connection.close_connection()
    ...
    def _create_eeg_signal(self, row: tuple) -> None:
        ...
        self.__eeg_signal.set_analysis_code(row[self.__ANALYSIS_CODE])
